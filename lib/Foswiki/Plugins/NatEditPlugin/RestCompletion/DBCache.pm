...
package Foswiki::Plugins::NatEditPlugin::RestCompletion::DBCache;
...
sub handleMention {
  ...
  my $search = "form=~'User(Form|Topic)\$' AND ( lc(topictitle)=~'^$star\Q$term\E' OR lc(topic)=~'^$star\Q$term\E' )"; # TODO: make user form configurable
  _writeDebug("search=$search");

  my $hits = $db->dbQuery($search, undef, "topictitle", "off");
...
sub handleTopic {
  ...
  my $db = Foswiki::Plugins::DBCachePlugin::getDB($termWeb);
  my $index = 0;
  my @result = ();

  my $search = "lc(topictitle)=~'^$star\Q$term\E' OR lc(topic)=~'^$star\Q$term\E'";
  _writeDebug("search=$search");
  ...
  while (my $obj = $hits->next) {
    my $topic = $obj->fastget("topic");
    my $topicTitle = $obj->fastget("topictitle");
  ...
  # TODO: get webs, same as in default -> move it to Base?
