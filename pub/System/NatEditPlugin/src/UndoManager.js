...
UndoManager.prototype.saveState = function(mode) {
  ...
  if (typeof(currentState) !== 'undefined') {
    ...
    if (currentState.value === self.shell.txtarea.value 
      ...
      self.shell.log("reuse current state in mode=",mode);
      self.mode = mode;
      currentState.init();
      return;
    }
  ...
  self.shell.log("mode=",mode);
  self.mode = mode;

  self.undoPtr++;
  self.shell.log("save state at undoPtr=",self.undoPtr);
...
UndoManager.prototype.undo = function() {
  ...
  self.shell.log("called undo");

  if (!self.canUndo()) {
    self.shell.log("... can't undo");
    return;
  }

  self.undoPtr--;
  self.shell.log("... undoing undoPtr=",self.undoPtr);
...
UndoManager.prototype.redo = function() {
  ...
  if (!self.canRedo()) {
    self.shell.log("... can't redo");
    return;
  }

  self.undoPtr++;
  self.shell.log("... redoing undoPtr=",self.undoPtr);
  self.undoBuf[self.undoPtr].restore();
